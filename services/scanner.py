...
def scan_barcodes(img: Image.Image) -> list[zxingcpp.Result]:
    """
    Сканировать штрих-коды с изображения.

    Делает несколько попыток с разными фильтрами:
    оригинал → контраст → резкость → увеличение x2.
    Возвращает результаты первой успешной попытки.
    """
    try:
        if img.mode != "RGB":
            img = img.convert("RGB")
